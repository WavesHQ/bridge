...
const DEFAULT_SCRYPT_N_R_P = [
  2 ** 14,
  9, // decide stress on ram, not to reduce, to remained strong POW
  3, // iteration, directly stack up time (if only purely single thread)
];
...
function initProvider(
  ...
  return EncryptedHdNodeProvider.init(data.raw, bip32Options, encryption, async () => promptInterface.prompt());
}
